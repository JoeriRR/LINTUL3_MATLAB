...
Cmax = [10 0; 0 8]; % agent max capacity matrix four agents that can deliver water and 1 agents that can deliver fertilizer
[r,n] = size(Cmax); % number of resources (r) and number of agents (n)
% Weighting vectors
pi    = 100;%rand(p,1); % price per product vector
rho   = [1; 100];%rand(r,1); % cost per resource vector (change second term)
sigma = ones(n,1); % cost per operation vector

U = [[linspace(0,10,4) zeros(1,3)]; [zeros(1,4) linspace(2,8,3)]]; % r*s
...
C_no_free_deli = [];
C_agent_cap    = [];
E = GetE_gui_grid(m,100);
A_init = [eye(n) zeros(n,m-n)];
for t = 1:q
    C_1fieldpagent = [C_1fieldpagent Atau(:,:,t)*ones(m,1) == 1  ];
    C_1agentpfield = [C_1agentpfield Atau(:,:,t)'*ones(n,1) <= 1 ];
    C_no_free_deli = [C_no_free_deli Dtau(:,:,t)*(eye(m)-diag(Atau(:,:,t)'*ones(n,1) )) == 0];
    C_agent_cap = [C_agent_cap Dtau(:,:,t)*Atau(:,:,t)' <= Cmax];
end
C_u_lo  = [];
C_u_hi  = [];
...
for t = 1:season_max_length-1
    ...
    for z = 1:q
        if z==1
            j_m1 = trace(Atau(:,:,q+1)*E*Atau(:,:,z)');
        else
            j_m2 = trace(Atau(:,:,z-1)*E*Atau(:,:,z)');
        end
    end
    j_q = kron(ones(m,1),rho')*sum(Dtau,3)+ kron(ones(m,1),sigma')*sum(Atau,3)+j_m1+j_m2;% rho'*sum(sum(Dtau,2),3) + sigma'*sum(sum(Atau,2),3);
    J = trace(j_g - j_l - j_q);
    optimize(C, -J ,ops);    
    D = round(value(delt));
    A{t} = round(value(Atau));   
    
    C(end) =[]; % delete initial previous
    C = [C, Atau(:,:,q+1) == A{t}(:,:,q)];
...
%% save results
Str = strcat('results/DHMARA_spatial_verylargeE_m', num2str(m),'_p', num2str(p),'_q',num2str(q),'_r',num2str(r),'_n',num2str(n));
...
count = 1;
while(true)
    GuiStr = ['results/gui_spatial' num2str(count) '.mat'];
